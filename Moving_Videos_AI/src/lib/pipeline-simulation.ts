import { PIPELINE_STEPS, DESIGN_STYLES, type PipelineStage, type LogEntry, type DesignStyle } from "@/lib/types";
...
function buildStageMessages(styleLabel: string): Record<string, { msg: string; type: LogEntry["type"] }[]> {
  return {
  color_grade: [
  ...
  ai_furnish: [
    ...
    { msg: `Style prompt injected: proportional ${styleLabel.toLowerCase()} furniture set`, type: "info" },
...
export async function simulatePipeline(
  ...
  const styleLabel = style ? DESIGN_STYLES[style].label : "Moderno";
  const STAGE_MESSAGES = buildStageMessages(styleLabel);
